...
import type { PositionSortField } from "@/entities/position";
...
interface PositionFiltersProps {
    ...
    onSortChange: (field: PositionSortField | null, descending: boolean) => void;
    onResetFilters: () => void;
}
...
export const PositionFilters = ({
    ...
    const deletedOptions: ComboboxOption[] = useMemo(() => [
        { value: "exclude", label: t("positions.filters.excludeDeleted") },
        { value: "include", label: t("positions.filters.includeDeleted") },
    ], [t]);

    const sortFieldOptions: ComboboxOption[] = useMemo(() => [
        { value: "none", label: t("positions.filters.sortFieldNone") },
        { value: "name", label: t("positions.filters.sortFieldName") },
        { value: "createdAt", label: t("positions.filters.sortFieldCreated") },
        { value: "updatedAt", label: t("positions.filters.sortFieldUpdated") },
    ], [t]);

    const sortDirectionOptions: ComboboxOption[] = useMemo(() => [
        { value: "asc", label: t("positions.filters.ascending") },
        { value: "desc", label: t("positions.filters.descending") },
    ], [t]);
    ...
    const handleSortFieldChange = (value: string) => {
        if (value === "none") {
            ...
            onSortChange(value as PositionSortField, sortDescending);
        }
    };
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    >
                        ...
                        <span className="hidden sm:block">{t("positions.filters.tabFilters")}</span>
                        {isFiltersExpanded ? (
                    ...
                    >
                        ...
                        <span className="hidden sm:block">{t("positions.filters.tabSorting")}</span>
                        {isSortingExpanded ? (
                            <ChevronUp className="h-4 w-4" />
                        ) : (
                            <ChevronDown className="h-4 w-4" />
                ...
                {hasAnyActive && (
                    ...
                    >
                        ...
                        {t("positions.filters.resetFilters")}
                    </Button>
                )}
            </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <div className="space-y-2">
                        <Label htmlFor="deleted-filter" className="text-xs font-medium">
                            {t("positions.filters.deletedPositions")}
                        </Label>
                        <Combobox
                            value={includeDeleted ? "include" : "exclude"}
                            onChange={handleDeletedChange}
                            options={deletedOptions}
                            aria-label={t("positions.filters.deletedPositions")}
                        />
                    </div>
                </div>
            )}
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <div className="space-y-2">
                        <Label htmlFor="sort-field" className="text-xs font-medium">
                            {t("positions.filters.sortField")}
                        </Label>
                        <Combobox
                            value={sortBy ?? "none"}
                            onChange={handleSortFieldChange}
                            options={sortFieldOptions}
                            aria-label={t("positions.filters.sortField")}
                        />
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="sort-direction" className="text-xs font-medium">
                            {t("positions.filters.sortDirection")}
                        </Label>
                        <Combobox
                            ...
                            disabled={!sortBy}
                            aria-label={t("positions.filters.sortDirection")}
                        />
                    </div>
                </div>
            )}
            ...
            {hasAnyActive && isAnyExpanded && (
                <div className="flex flex-wrap gap-2 pt-2">
                    <span className="text-xs text-muted-foreground">
                        {t("positions.filters.activeFilters")}:
                    </span>
                    {sortBy && (
                    ...
                    {includeDeleted && (
                        <Badge variant="secondary" className="text-xs">
                            {t("positions.filters.includeDeleted")}
                            <button
                                type="button"
